...
const skillPath = path.resolve('.agents/skills/aisdk-devtools-generations/SKILL.md');
...
describe('repo-local skill', () => {
  it('documents the safe agent workflow without installing to personal skills', () => {
    ...
    expect(skill).toContain('Prefer `messages`, `steps`, `output`, `tools`, and `usage` before `raw`.');
    expect(skill).toContain('Use `raw --json-path` before `raw --full`; quote JSON paths');
